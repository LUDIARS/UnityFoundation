...
## Melpomene Runtime(未検証)
ビルド後のゲーム内で動くバグ報告フォーム(IMGUI オーバーレイ)。
送信先は**選択式**で、GitHub Issues API への直送(GitHubDirect)に加え、
トークンをクライアントへ載せない任意のリレーサーバ(Relay)へも送れる。
[[MelpomeneRuntime]]
[[MelpomeneDestination]]